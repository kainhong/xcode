...
public class TemplateSettingForm implements Configurable, BaseSettingsFrom<GenerateStateStorage> {
    ...
    private void initGroupName() {
        ...
        this.groupNameComponent = new GroupNameComponent<>(switchGroupOperator,
                this.templateGroupMap,
                Template::new);
    ...
    private void initEditList() {
        ...
        this.editListComponent = new EditListComponent<>(switchItemFun, "Template Name:",
                Template.class, this.currTemplateGroup.getItems());
    }
    ...
    @Override
    public GenerateStateStorage getSettingsStorage() {
        return null;
    }
    ...
    public void loadSettingsStore(GenerateStateStorage GenerateStateStorage) {
        ...
        for (TemplateGroup templateGroup : TemplateConfiguration.instance().getTemplateGrops()) {
            templateGroup = CloneUtils.cloneByJson(templateGroup);
            templateGroupMap.put(templateGroup.getName(), templateGroup);
        }


        this.currTemplateGroup = TemplateConfiguration.instance().getTemplateGrops().get(0);
...
//        this.loadSettingsStore(getGenerateStateStorage());
    ...
    private void refreshUiVal() {
        ...
        if (this.editListComponent != null) {
            this.editListComponent.setElementList(this.currTemplateGroup.getItems());
        }
    }
}
