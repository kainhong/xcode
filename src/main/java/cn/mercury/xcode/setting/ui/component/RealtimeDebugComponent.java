...
import cn.mercury.xcode.utils.VelocityUtils;
...
public class RealtimeDebugComponent {
    ...
    private void runDebug() {
        ...
        // 获取表信息
        TableInfo tableInfo = TableInfo.from(dbTable);
        // 为未配置的表设置一个默认包名
//        if (tableInfo.getSavePackageName() == null) {
//            tableInfo.setSavePackageName("com.companyname.modulename");
//        }

        var project = ProjectUtils.getCurrProject();

        CodeGenerateService service = new CodeGenerateService(project);

        Template template = new Template("temp", editorComponent.getFile().getCode());

        String code = service.generate(template,tableInfo);
